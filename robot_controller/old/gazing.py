...
def reset_pids(Kp=1.0, Ki=0.0, Kd=0.0):
        ...
        # Ki, Kd = (0.05, 0.0)
        pid_w1 = PID(3.0, Ki, Kd, setpoint=0.0)
...
class Gazer:
    ...
    def desired_joint_angles(self, target, head_tranformation_matrix, joint_states):
        try:
            ...
            # if not isinstance(e, np.linalg.LinAlgError):
            #     print("Error calculating desired joint angles: ", e)
    ...
    def step(self, target, head_tranformation_matrix, joint_states):
        ...
        for pid in self.pids:
            pid.setpoint = desired_joints.pop(0)
        return [pid(joint_states[i+3]) for i, pid in enumerate(self.pids)]
    
    def update_pid(self, param_list):
        for i, pid in enumerate(self.pids):
            self.pids[i].tunings = param_list
